...
coverage:
	Rscript -e 'covr::package_coverage(path=".", line_exclusions=c(file.path("packrat",  list.files(path="packrat", recursive=TRUE)), "src/jsoncpp.cpp", "src/json/json.h"))'

codecov:
	Rscript -e 'covr::codecov(path=".", line_exclusions=c(file.path("packrat",  list.files(path="packrat", recursive=TRUE)), "src/jsoncpp.cpp", "src/json/json.h"))'
