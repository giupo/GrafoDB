...
test_that("tsdiff returns TRUE if two timeseries differ",  {
  a <- TSERIES(runif(10), START=c(1990,1), FREQ=1)
  b <- TSERIES(runif(10), START=c(1990,1), FREQ=1)
  expect_true(tsdiff(a,b))
  a <- TSERIES(runif(10), START=c(1990,1), FREQ=4)
  b <- TSERIES(runif(10), START=c(1990,1), FREQ=4)
  expect_true(tsdiff(a,b))
  a <- TSERIES(runif(10), START=c(1990,1), FREQ=12)
  b <- TSERIES(runif(10), START=c(1990,1), FREQ=12)
  expect_true(tsdiff(a,b))
})
...
test_that("tsdiff returns TRUE if two timeseries have different dimensions",  {
  v1 <- c(1,2,3,4,5)
  v2 <- c(v1, 6)
  a <- TSERIES(v1, START=c(1990,1), FREQ=1)
  b <- TSERIES(v2, START=c(1990,1), FREQ=1)
  expect_true(tsdiff(a,b))
  a <- TSERIES(v1, START=c(1990,1), FREQ=4)
  b <- TSERIES(v2, START=c(1990,1), FREQ=4)
  expect_true(tsdiff(a,b))
  a <- TSERIES(v1, START=c(1990,1), FREQ=12)
  b <- TSERIES(v2, START=c(1990,1), FREQ=12)
  expect_true(tsdiff(a,b))
})
